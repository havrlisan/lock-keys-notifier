...
- **`Wh_ModInit`** — load settings; start the worker thread, which creates the
  ...
  - **Cold-start warm-up (added 1.0.1):** after the worker installs the hook and
    signals ready, it spawns a **throwaway warm-up thread** that does **one
    off-screen `RenderToast`** into a scratch DIB, then frees it. The first GDI+
    text render pays a one-time lazy cost — font enumeration, loading the
    configured family, spinning up the GDI+ text subsystem and its DLLs — which on
    a **cold boot** (cold disk) can be several seconds. Warming it moves that cost
    off the first key press. It only manifested on cold boot, not disable/enable,
    because the OS font/DLL caches stay warm across an enable cycle. (Issue
    [#4555](https://github.com/ramensoftware/windhawk-mods/issues/4555).)
    - **Why a separate thread (refined post-review):** the worker thread owns the
      `WH_KEYBOARD_LL` hook and must keep pumping messages, or every system-wide
      keystroke stalls on the `LowLevelHooksTimeout` (~300 ms) until the hook
      returns — a multi-second warm-up there would degrade global keyboard input
      on every boot. GDI+ is initialized per-process and the caches it warms are
      process-wide, so warming on any thread benefits the worker's first real
      render. The worker joins the warm-up thread before `GdiplusShutdown` so it
      can't draw post-teardown, and skips spawning it entirely when no `notify*`
      key is enabled (nothing would ever render).
