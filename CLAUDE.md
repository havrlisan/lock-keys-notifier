...
- **Startup handshake.** `StartWorker` creates a manual-reset event; the worker seeds key states, installs the hook, sets `g_hookInstalled`, and `SetEvent`s **unconditionally** before entering the pump. `StartWorker` waits on that event and returns `g_hookInstalled`, so `Wh_ModInit` returns `FALSE` when the hook fails to install. Shutdown posts `WM_APP_QUIT` as a **window** message (reliable; the window is guaranteed created once the worker signaled ready) → `PostQuitMessage`. Preserve this ordering — moving the hook install or the `SetEvent` can reintroduce a hang or a dropped-quit thread leak.
- **State detection.** Lock state is self-tracked: seed from `GetKeyState(vk) & 1` at startup, then flip the tracked bool on the **key-down edge only** (a per-key `g_keyDown` flag ignores auto-repeat). This avoids a `GetKeyState`-lag race. `g_keyState`/`g_keyDown` are touched only by the worker thread once events flow.
